...
fn print_verification_instructions() {
    ...
    println!("   tpm2_nvread -T mssim:host=localhost,port=2321 -C o {:#x} -o ek_cert.der", NV_INDEX_EK_CERT);
    ...
    println!("   openssl verify -CAfile {} -untrusted ek_cert.der ek_cert.der", CA_CERT_FILENAME);
...
fn connect_and_reset_tpm() -> Result<Context> {
    ...
    */
    ...
    ctx.startup(StartupType::Clear).context("Failed to start up TPM")?;
...
fn write_cert_to_file(ca_cert: &X509, filename: &str) -> Result<()>{
    ...
    file.write_all(&pem).context("Failed to write CA cert to file")?;
...
fn write_cert_to_nvram(context: &mut Context, cert_data: &[u8]) -> Result<()> {
    ...
    let nv_handle_obj = match context.nv_define_space(
                                                      Provision::Owner,
                                                      None,
                                                      nv_public) {
        Ok(h) => h,
        Err(e) => {
            ...
            if err_msg.contains("0x14c") || err_msg.contains("already defined") {
                println!("NV Index {:#x} already exists. Skipping write.", NV_INDEX_EK_CERT);
                return Ok(());
            }
            return Err(e.into());
        }
    ...
    println!("Stored Certificate in NVRAM (Index: {:#x}).", NV_INDEX_EK_CERT);
    Ok(())
}
...
fn verify_nvram_index(context: &mut Context) -> Result<()> {
    ...
    let (data, _) = context.get_capability(
    ...
    match data {
        tss_esapi::structures::CapabilityData::Handles(handles) => {
            if handles.iter().any(|h| match h {
                ...
                bail!("Verification Failed: Index {:#x} not found.", NV_INDEX_EK_CERT)
            }
        },
